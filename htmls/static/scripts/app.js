...
(function($, window, document) {
	...
	// get the init carousel
	var trueCarousel = $('.carousel').html();

	// listener for hamburger launcher
	$('header.main nav .launcher').on('click', function(e) {
		e.preventDefault();
		$('header.main nav > ul').stop().slideToggle();
	});
	...
	function initCarousel(carousel) {
		carousel.jCarouselLite({
			init: function(options, slides) {
				var ctrl = $('.carousel-ctrl'),
					slide_count = slides.length;

				if (slides && slide_count > 0) {

					for (max=slide_count, i=1; i<=slide_count; i++) {
						ctrl.append($('<a href="#"'+i+'/>').text(i));
					}
				}

				options.btnGo = $('.carousel-ctrl a');
			},
			visible:1,
			auto:true,
			speed:800
	...
	function lazyCarousel() {
		...
		// destroy carousel
		$('.carousel').jCarouselLite = null;
		$('.carousel').attr('style', '');
		$('.carousel, .carousel-ctrl').html('');
		$('.carousel').html(trueCarousel);
	...
	// var lazyLayout = _.debounce(lazyCarousel, 500);
	// $(window).on('resize', lazyCarousel);
